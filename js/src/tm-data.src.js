...
// de línea con trazo fino, como el preview, en vez de emoji. SIEMPRE devuelve
// un SVG: primero intenta por nombre, luego por el emoji efectivo de la
// categoría (personalizado o automático), y si nada coincide cae en un ícono
// genérico de tienda — así CUALQUIER categoría/emoji que elija el admin sale
// con el mismo diseño de línea. OJO: gana sobre iconosPersonalizados a
// propósito — categorias.json publica un set de emojis por defecto que llega
// a TODOS los visitantes vía ese mismo mecanismo, y son exactamente el diseño
// viejo que este rediseño sustituye. Los paths son constantes propias — nunca
// entrada de usuario — así que es seguro inyectarlos con innerHTML sin escapar.
// Cada entrada mezcla claves de texto (contra el nombre) y emojis (contra el
// ícono elegido); los emojis van sin selector de variación para que
// claves como '🛠' matcheen también la forma '🛠️'.
...
const ICONOS_SVG_MAPA = [
    ...
    [['belleza', 'maquillaje', 'perfume', 'cosmetic', '💄', '💅', '🧴', '✨'], '<path d="M9.94 15.5 8.5 14.06 2.36 12.48a.5.5 0 0 1 0-.96L8.5 9.94 9.94 8.5l1.58-6.14a.5.5 0 0 1 .96 0l1.58 6.14 1.44 1.44 6.14 1.58a.5.5 0 0 1 0 .96l-6.14 1.58-1.44 1.44-1.58 6.14a.5.5 0 0 1-.96 0z"/>'],
    [['libro', 'papeleria', 'escuela', 'estudio', '📚', '📖', '✏'], '<path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/>'],
    [['reloj', 'joya', 'accesorio', '⌚', '💎', '💍'], '<circle cx="12" cy="12" r="6"/><polyline points="12 10 12 12 13 13"/><path d="m16.13 7.66-.81-4.05a2 2 0 0 0-2-1.61h-2.68a2 2 0 0 0-2 1.61l-.78 4.05"/><path d="m7.88 16.36.8 4a2 2 0 0 0 2 1.61h2.72a2 2 0 0 0 2-1.61l.81-4.05"/>'],
    [['mascota', 'perro', 'gato', '🐾', '🐶', '🐱'], '<circle cx="11" cy="4" r="2"/><circle cx="18" cy="8" r="2"/><circle cx="20" cy="16" r="2"/><path d="M9 10a5 5 0 0 1 5 5v3.5a3.5 3.5 0 0 1-6.84 1.045Q6.52 17.48 4.46 16.84A3.5 3.5 0 0 1 5.5 10Z"/>'],
...
// Respaldo genérico (bolsa de tienda): garantiza que ninguna tarjeta caiga
// al emoji crudo aunque ni el nombre ni el emoji matcheen nada.
const _SVG_CAT_GENERICO = '<path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/>';
