(function(){"use strict";if(window._tmBotLoaded||location.pathname.includes("/admin"))return;window._tmBotLoaded=!0;function k(){const t=document.querySelector('meta[name="tm-bot-cerebro"]');return t&&t.content?t.content:"js/tm-bot-cerebro.js"}let n=!1,p=!1,f=!1,x=!1;const u=document.createElement("style");u.textContent=`
    ...
    `,document.head.appendChild(u);const e=document.createElement("button");e.className="tm-bot-bubble",e.id="tmBotBubble",e.setAttribute("aria-label","Abrir chat con Max"),e.innerHTML='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C6.48 2 2 6.48 2 12c0 1.86.51 3.6 1.39 5.09L2 22l4.91-1.39C8.4 21.49 10.14 22 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2zm0 18c-1.66 0-3.22-.45-4.56-1.24l-.33-.2-2.92.83.83-2.92-.2-.33C3.45 15.22 3 13.66 3 12c0-4.96 4.04-9 9-9s9 4.04 9 9-4.04 9-9 9zm5-9.5c0 2.49-2.01 4.5-4.5 4.5-.78 0-1.51-.2-2.15-.55L8 15.5l1.05-2.35C8.7 12.51 8.5 11.78 8.5 11c0-2.49 2.01-4.5 4.5-4.5S17 8.51 17 11z"/></svg><span class="tm-bot-badge" aria-hidden="true">1</span>',document.body.appendChild(e);const r=document.createElement("div");r.className="tm-bot-welcome",r.id="tmBotWelcome",r.innerHTML='<button class="tm-bot-welcome-close" aria-label="Cerrar">\xD7</button><div class="tm-bot-welcome-avatar">\u{1F916}</div><div class="tm-bot-welcome-txt"><b>Max \xB7 Asistente</b><br>\xBFBuscas algo? Preg\xFAntame lo que sea \u{1F44B}</div>',document.body.appendChild(r);const o=document.createElement("div");o.className="tm-bot-panel",o.id="tmBotPanel",o.setAttribute("role","dialog"),o.setAttribute("aria-label","Chat con Max, asistente de TiendaMax"),o.innerHTML=`
    ...
    `,document.body.appendChild(o);const c=o.querySelector("#tmBotBody"),s=o.querySelector("#tmBotInput"),C=o.querySelector("#tmBotClose"),h=r.querySelector(".tm-bot-welcome-close");function w(t,i){const a=document.createElement("div");return a.className="tm-bot-msg bot",a.id=i||"",a.innerHTML=t,c.appendChild(a),c.scrollTop=c.scrollHeight,a}function l(){if(window._tmBotCerebroListo||p)return;p=!0,f=!1;const t=document.createElement("script");t.src=k(),t.defer=!0,t.onerror=()=>{p=!1,f=!0;const i=document.getElementById("tmBotCargando");i&&i.remove();const a=typeof getNumeroWhatsApp=="function"?getNumeroWhatsApp():"5354320170";w('\u{1F615} No pude cargar el asistente (parece que la conexi\xF3n fall\xF3). Puedes reintentar o escribirnos directo por WhatsApp: <a class="tm-bot-link" href="https://wa.me/'+a+'" target="_blank" rel="noopener noreferrer">wa.me/'+a+"</a>");const m=document.getElementById("tmBotQuickReplies");if(m){m.innerHTML="";const g=document.createElement("button");g.className="tm-bot-quickreply",g.textContent="\u{1F504} Reintentar",g.onclick=()=>{m.innerHTML="",c.innerHTML="",d()},m.appendChild(g)}},document.head.appendChild(t)}["mouseenter","touchstart","focus"].forEach(t=>{e.addEventListener(t,l,{once:!0,passive:!0})}),window._tmBotAlCargarCerebro=function(){p=!1;const t=document.getElementById("tmBotCargando");if(t&&t.remove(),!x)return;try{window._tmBot.open()}catch(a){}const i=window._tmBotPendiente;if(window._tmBotPendiente=null,i)try{window._tmBot.send(i)}catch(a){}};function d(){x=!0,o.classList.add("open"),n=!0,e.classList.remove("has-new");const t=e.querySelector(".tm-bot-badge");if(t&&t.remove(),r.classList.remove("visible"),window._tmBotCerebroListo){try{window._tmBot.open()}catch(i){}return}!document.getElementById("tmBotCargando")&&!f&&w("\u{1F44B} Un segundo, estoy despertando\u2026","tmBotCargando"),l(),setTimeout(()=>{try{s.focus()}catch(i){}},300)}function b(){o.classList.remove("open"),n=!1}e.onclick=()=>{n?b():d()},C.onclick=b,h.onclick=t=>{t.stopPropagation(),r.classList.remove("visible")},r.addEventListener("click",t=>{t.target!==h&&d()});function y(){if(window._tmBotCerebroListo)return;const t=(s.value||"").trim();t&&(window._tmBotPendiente=t,s.value=""),l()}o.querySelector("#tmBotSend").onclick=y,s.addEventListener("focus",l,{once:!0}),s.addEventListener("keydown",t=>{t.key==="Enter"&&!window._tmBotCerebroListo&&(t.preventDefault(),y())}),document.addEventListener("keydown",t=>{t.key==="Escape"&&n&&!window._tmBotCerebroListo&&b()});function v(){sessionStorage.getItem("tm_bot_welcome_shown")||setTimeout(()=>{n||(sessionStorage.setItem("tm_bot_welcome_shown","1"),r.classList.add("visible"),e.classList.add("has-new"),setTimeout(()=>{n||r.classList.remove("visible")},9e3))},4e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v):v(),window._tmBot=window._tmBot||{open:d,close:b,send:t=>{d(),window._tmBotPendiente=t}},window._tmBotCascara={abrir:d,cerrar:b,cargarCerebro:l}})();
