(function(){"use strict";if(window._tmBotLoaded||location.pathname.includes("/admin"))return;window._tmBotLoaded=!0;function _(){const t=document.querySelector('meta[name="tm-bot-cerebro"]');return t&&t.content?t.content:"js/tm-bot-cerebro.js"}let d=!1,x=!1,w=!1,k=!1;const C=document.createElement("style");C.textContent=`
    ...
    /* .tm-bot-body es un flex column con scroll, as\xED que sus hijos se
       encogen por defecto (flex-shrink:1) en cuanto la conversaci\xF3n pasa
       del alto del panel. La tabla comparativa, que no tiene padding
       propio que la sostenga, quedaba aplastada a 2px de alto: la funci\xF3n
       estrella del bot se ve\xEDa como una rayita. */
    .tm-bot-body > *{flex-shrink:0;}
    /* \u2500\u2500 Mensajes \u2500\u2500
       white-space:pre-wrap es obligatorio: las respuestas se arman como
       texto con \\n (listas de vi\xF1etas, tablas de potencias, fichas). Sin
    ...
    /* Quien pidi\xF3 menos animaci\xF3n no deber\xEDa recibir una burbuja que late
    ...
    `,document.head.appendChild(C);const e=document.createElement("button");e.className="tm-bot-bubble",e.id="tmBotBubble",e.setAttribute("aria-label","Abrir chat con Max"),e.innerHTML='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C6.48 2 2 6.48 2 12c0 1.86.51 3.6 1.39 5.09L2 22l4.91-1.39C8.4 21.49 10.14 22 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2zm0 18c-1.66 0-3.22-.45-4.56-1.24l-.33-.2-2.92.83.83-2.92-.2-.33C3.45 15.22 3 13.66 3 12c0-4.96 4.04-9 9-9s9 4.04 9 9-4.04 9-9 9zm5-9.5c0 2.49-2.01 4.5-4.5 4.5-.78 0-1.51-.2-2.15-.55L8 15.5l1.05-2.35C8.7 12.51 8.5 11.78 8.5 11c0-2.49 2.01-4.5 4.5-4.5S17 8.51 17 11z"/></svg><span class="tm-bot-badge" aria-hidden="true">1</span>',document.body.appendChild(e);const c=document.createElement("button");c.type="button",c.className="tm-bot-label",c.textContent="Preg\xFAntale a Max",c.addEventListener("click",()=>e.click()),document.body.appendChild(c);const i=document.createElement("div");i.className="tm-bot-welcome",i.id="tmBotWelcome",i.innerHTML='<button class="tm-bot-welcome-close" aria-label="Cerrar">\xD7</button><div class="tm-bot-welcome-avatar">\u{1F916}</div><div class="tm-bot-welcome-txt"><b>Max \xB7 Asistente</b><br>\xBFBuscas algo? Preg\xFAntame lo que sea \u{1F44B}</div>',document.body.appendChild(i);const r=document.createElement("div");r.className="tm-bot-panel",r.id="tmBotPanel",r.setAttribute("role","dialog"),r.setAttribute("aria-label","Chat con Max, asistente de TiendaMax"),r.innerHTML=`
        <div class="tm-bot-header">
            ...
            <div class="tm-bot-header-info">
                ...
                <div class="tm-bot-header-status">En l\xEDnea \xB7 responde al instante</div>
            </div>
            <div class="tm-bot-header-actions">
                <button class="tm-bot-header-btn" id="tmBotReset" title="Reiniciar conversaci\xF3n" aria-label="Reiniciar conversaci\xF3n">
        ...
        <div class="tm-bot-input">
            <div class="tm-bot-input-wrap">
                <input type="text" id="tmBotInput" placeholder="Preg\xFAntame lo que sea\u2026" autocomplete="off" aria-label="Escribe tu mensaje">
    ...
    `,document.body.appendChild(r);const f=r.querySelector("#tmBotBody"),m=r.querySelector("#tmBotInput"),q=r.querySelector("#tmBotClose"),E=i.querySelector(".tm-bot-welcome-close");function B(t,a){const o=document.createElement("div");return o.className="tm-bot-msg bot",o.id=a||"",o.innerHTML=t,f.appendChild(o),f.scrollTop=f.scrollHeight,o}function g(){if(window._tmBotCerebroListo||x)return;x=!0,w=!1;const t=document.createElement("script");t.src=_(),t.defer=!0,t.onerror=()=>{x=!1,w=!0;const a=document.getElementById("tmBotCargando");a&&a.remove();const o=typeof getNumeroWhatsApp=="function"?getNumeroWhatsApp():"5354320170";B('\u{1F615} No pude cargar el asistente (parece que la conexi\xF3n fall\xF3). Puedes reintentar o escribirnos directo por WhatsApp: <a class="tm-bot-link" href="https://wa.me/'+o+'" target="_blank" rel="noopener noreferrer">wa.me/'+o+"</a>");const l=document.getElementById("tmBotQuickReplies");if(l){l.innerHTML="";const b=document.createElement("button");b.className="tm-bot-quickreply",b.textContent="\u{1F504} Reintentar",b.onclick=()=>{l.innerHTML="",f.innerHTML="",s()},l.appendChild(b)}},document.head.appendChild(t)}["mouseenter","touchstart","focus"].forEach(t=>{e.addEventListener(t,g,{once:!0,passive:!0})}),window._tmBotAlCargarCerebro=function(){x=!1;const t=document.getElementById("tmBotCargando");if(t&&t.remove(),!k)return;try{window._tmBot.open()}catch(o){}const a=window._tmBotPendiente;if(window._tmBotPendiente=null,a)try{window._tmBot.send(a)}catch(o){}};function s(){k=!0,r.classList.add("open"),d=!0,e.classList.remove("has-new");const t=e.querySelector(".tm-bot-badge");if(t&&t.remove(),i.classList.remove("visible"),window._tmBotCerebroListo){try{window._tmBot.open()}catch(a){}return}!document.getElementById("tmBotCargando")&&!w&&B("\u{1F44B} Un segundo, estoy despertando\u2026","tmBotCargando"),g(),setTimeout(()=>{try{m.focus()}catch(a){}},300)}function u(){r.classList.remove("open"),d=!1}e.onclick=()=>{d?u():s()},q.onclick=u,E.onclick=t=>{t.stopPropagation(),i.classList.remove("visible")},i.addEventListener("click",t=>{t.target!==E&&s()});function z(){if(window._tmBotCerebroListo)return;const t=(m.value||"").trim();t&&(window._tmBotPendiente=t,m.value=""),g()}r.querySelector("#tmBotSend").onclick=z,m.addEventListener("focus",g,{once:!0}),m.addEventListener("keydown",t=>{t.key==="Enter"&&!window._tmBotCerebroListo&&(t.preventDefault(),z())}),document.addEventListener("keydown",t=>{t.key==="Escape"&&d&&!window._tmBotCerebroListo&&u()});function L(){sessionStorage.getItem("tm_bot_welcome_shown")||setTimeout(()=>{d||(sessionStorage.setItem("tm_bot_welcome_shown","1"),e.classList.add("has-new"))},4e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",L):L(),window._tmBot=window._tmBot||{open:s,close:u,send:t=>{s(),window._tmBotPendiente=t}},window._tmBotCascara={abrir:s,cerrar:u,cargarCerebro:g};const D=".btn-pedir-card,.btn-pedir-whatsapp,.carrito-btn-comprar";let h=0,y=!1;function M(){if(y=!1,d)return;const t=e.getBoundingClientRect(),a=t.top+h,o=t.bottom+h,l=t.left-6,b=t.right+6;let n=0;document.querySelectorAll(D).forEach(S=>{const p=S.getBoundingClientRect();!p.width||p.bottom<a-6||p.top>o+6||p.right<l||p.left>b||(n=Math.max(n,o-p.top+10))}),n=Math.min(Math.round(n),140),n!==h&&(h=n,document.documentElement.style.setProperty("--tmb-esq",n+"px"))}function v(){y||(y=!0,requestAnimationFrame(M))}document.addEventListener("scroll",v,{passive:!0,capture:!0}),window.addEventListener("resize",v,{passive:!0}),setTimeout(v,1500)})();
