...
const PUSH_BANNER_DELAY_MS = 45000; 
const PUSH_BANNER_DENY_DELAY_HOURS = 6; 
const PUSH_RECHAZO_DELAY_DAYS = [1, 3, 7, 14, 30]; 
...
function limpiarRecientes() {
    ...
    try { localStorage.removeItem('recientes'); } catch(e) {}  
    renderizarRecientes();
}
...
async function subirImagenAGitHub(fileOrBase64) {
    ...
    if (!user || !repo || !token) return base64full; 

    try {
        const base64data = base64full.includes(',') ? base64full.split(',')[1] : base64full;
        if (!base64data) return base64full; 
        ...
        if (dataProd && dataProd.length > 0) {
            ...
            productos = dataProd.map(p => {
                const fix = url => url && url.includes('raw.githubusercontent.com')
                    ? url.replace(/https:\/\/raw\.githubusercontent\.com\/[^/]+\/[^/]+\/main\
                    : url;
...
function abrirDetalleProducto(id) {
    ...
    if (typeof tmTrackVista === 'function') tmTrackVista(id); 
...
function registrarVenta(productoId, cantidad) {
    ...
    guardarVenta(venta);
    ajustarStock(productoId, -(cantidad || 1), true); 
    renderizarVentas();
...
function agregarGrupoFB() {
    ...
    grupos.push({ url: '', productos: productos.map(p => p.id) }); 
...
async function solicitarYRegistrarTokenFCM(messaging, config, fcmReg) {
    try {
        ...
        if (token) {
            ...
            const legacyTokenId = btoa(token).replace(/=/g, '').replace(/\+/g, '-').replace(/\
        ...
        if (tokenLocal) {
            try {
                ...
                if (fbConfig && fbConfig.databaseURL) {
                    const legacyTokenId = btoa(tokenLocal).replace(/=/g,'').replace(/\+/g,'-').replace(/\
