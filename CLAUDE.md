...
- `CMP_OCULTOS` (`tm_cmp_ocultos`) is the plain dismissal. Both are listed and undoable in the block's footer — a list that hides things without saying how many or which is a list nobody trusts twice. Both live in this browser only, like the panel's other local state.
