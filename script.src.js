...
function abrirDetalleProducto(id) {
    console.log('[DEBUG] abrirDetalleProducto llamado con id:', id);
    const p = productos.find(prod => prod.id === id);
    if (!p) {
        console.error('[DEBUG] Producto no encontrado:', id);
        return;
    }
    console.log('[DEBUG] Producto encontrado:', p.nombre);
...
if (_detailPrecioOldEl) {
    ...
    console.log('[DEBUG] Modal tiene clase hidden antes:', modal.classList.contains('hidden'));
    modal.classList.remove('hidden');
    modal.style.removeProperty('display');
    document.body.style.overflow = 'hidden';
    console.log('[DEBUG] Modal tiene clase hidden después:', modal.classList.contains('hidden'));
        ...
        navigator.serviceWorker.register('/firebase-messaging-sw.js', { scope: '/firebase-cloud-messaging-push-scope' })
            .then(fcmReg => {
                ...
                if (Notification.permission === 'granted') {
                    ...
                    if (desuscrito) {
                        console.log('[FCM] Usuario desuscrito manualmente, no se registra token.');
